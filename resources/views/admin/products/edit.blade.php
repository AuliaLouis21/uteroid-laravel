            ...
            <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="category_id" id="category_id" class="form-select @error('category_id') is-invalid @enderror">
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat->id }}" {{ old('category_id', $product->product_category_id) == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                            @endforeach
                        </select>
                        @error('category_id')
                            <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="size" id="size" class="form-control @error('size') is-invalid @enderror" value="{{ old('size', $product->size) }}" placeholder="cth: 40x30 cm">
                        @error('size')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="size_unit" id="size_unit" class="form-select @error('size_unit') is-invalid @enderror">
                            ...
                            <option value="cm2" {{ old('size_unit', $product->size_unit) === 'cm2' ? 'selected' : '' }}>Cm² (centimeter persegi)</option>
                            <option value="m2" {{ old('size_unit', $product->size_unit) === 'm2' ? 'selected' : '' }}>m² (meter persegi)</option>
                        </select>
                        @error('size_unit')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" name="thickness" id="thickness" class="form-control @error('thickness') is-invalid @enderror" value="{{ old('thickness', $product->thickness) }}" placeholder="cth: 1 cm">
                        @error('thickness')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mb-3">
                    ...
                    @if($product->image)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" width="120" height="120" style="object-fit:cover; border-radius:4px;">
                        </div>
                    @endif
                    ...
                    @error('image')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <small class="text-muted">Max 2MB. Format: JPG, JPEG, PNG, WebP. Leave empty to keep current image.</small>
                </div>
                ...
                <div class="mb-3 form-check">
                    ...
                    <input type="checkbox" name="is_promo" id="is_promo" value="1" class="form-check-input" {{ old('is_promo', $product->is_promo) ? 'checked' : '' }}>
