	...
	<table border='1' style='float: left;border-collapse: collapse; border: 1px solid silver; width: 100%;'>
		...
		<? foreach($query->result_array() as $row) { ?>
			<tr>
				...
				<td class='table-row-center'><?= label_for(date('d M Y',time($row['dtpDesain']))) ?></td>
				<td class='table-row-center'><?= label_for(date('d M Y',time($row['dtpProduksi']))) ?></td>
				<td class='table-row-center'><?= label_for(date('d M Y',time($row['dtpKlien']))) ?></td>
