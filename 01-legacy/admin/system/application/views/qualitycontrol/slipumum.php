	...
	<table border='1' style='float: left;border-collapse: collapse; border: 1px solid silver; width: 100%;'>
		...
		<? foreach($query->result_array() as $row) { ?>
			<tr>
				...
				<td class='table-row-center'><?= label_for($row['no_slip']) ?></td>
				<td class='table-row-left'><?= label_for($row['nama_klien']) ?></td>
				<td class='table-row-center'><?= label_for(date('d M Y',time($row['tgl_desain']))) ?></td>
				<td class='table-row-center'><?= label_for(date('d M Y',time($row['tgl_produksi']))) ?></td>
				<td class='table-row-center'><?= label_for(date('d M Y',time($row['tgl_selesai']))) ?></td>
