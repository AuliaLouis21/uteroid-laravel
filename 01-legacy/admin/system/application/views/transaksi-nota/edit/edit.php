<html>
	<head>
		<title>Web Admistrator ::: TRANSAKSI NOTA ::: EDITING NOTA <?php echo $row_order['nota'] ?></title>
		...
		<?php $this->load->view('transaksi-nota/edit/edit-transaksi.js.php') ?>
		<?php $this->load->view('transaksi-nota/edit/init.js.php') ?>
	</head>
	<body>
		<div id="header">
      <?php
			...
			<div id='contentwrapper'>
									...
									<fieldset>
										...
										<table style='float:left;'>
											<tr>
												...
												<td colspan='3'><?php echo textbox_tag(array('id'=>'no_nota','name'=>'no_nota','value'=>$row_order['nota']))?></td>
												
											</tr>
											<tr>
												<td><?php echo label_for('Nama')?></td>
												<td><?php echo label_for(':')?></td>
												<td><?php echo textbox_tag(array('id'=>'nama','name'=>'nama','style'=>'width:220px','value'=>$row_klien['nama']))?></td>
											...
											<tr>
												...
												<td><?php echo label_for(':')?></td>
												<td colspan='3'><?php echo textbox_tag(array('id'=>'alamat','name'=>'alamat','style'=>'width:220px','value'=>$row_klien['alamat']))?></td>
											</tr>
											<tr>
												<td><?php echo label_for('Telepon')?></td>
												<td><?php echo label_for(':')?></td>
												<td colspan='3'><?php echo textbox_tag(array('id'=>'telepon','name'=>'telepon','value'=>$row_klien['telepon']))?></td>
											</tr>
											<tr>
												<td><?php echo label_for('Email')?></td>
												<td><?php echo label_for(':')?></td>
												<td colspan='3'><?php echo textbox_tag(array('id'=>'email','name'=>'email','value'=>$row_klien['email']))?></td>
											</tr>
											<tr>
												<td><?php echo label_for('Perusahaan')?></td>
												<td><?php echo label_for(':')?></td>
												<td colspan='3'><?php echo textbox_tag(array('id'=>'perusahaan','name'=>'perusahaan','style'=>'width:220px','value'=>$row_klien['perusahaan']))?></td>
											</tr>
											<tr>
												<td><?php echo label_for('Tema')?></td>
												<td><?php echo label_for(':')?></td>
												<td colspan='3'><?php echo textbox_tag(array('id'=>'tema','name'=>'tema','style'=>'width:220px','value'=>$row_order['tema']))?></td>
											</tr>
										</table>
									</fieldset>
								</td>
									...
									<fieldset>
										...
										<table style='float:left'>
											<tr>
												...
												<td><?php echo label_for(':')?></td>
												<td><?php echo textbox_tag(array('id'=>'nama_sales','name'=>'nama_sales','value'=>$row_sales['nama']))?></td>
											...
											<tr>
												...
												<td><?php echo label_for(':')?></td>
												<td colspan='3'><?php echo calendar_tag(array('id'=>'tanggal_terima','name'=>'tanggal_terima','value'=>date('d-m-y',time($row_order['tgl_terima']))))?></td>
											</tr>
											<tr>
												<td><?php echo label_for('DeadLine Desain')?></td>
												<td><?php echo label_for(':')?></td>
												<td colspan='3'><?php echo calendar_tag(array('id'=>'deadline_desain','name'=>'deadline_desain','value'=>date('d-m-y',time($row_order['tgl_desain']))))?></td>
											</tr>
											<tr>
												<td><?php echo label_for('Dealine Selesai')?></td>
												<td><?php echo label_for(':')?></td>
												<td colspan='3'><?php echo calendar_tag(array('id'=>'deadline_selesai','name'=>'deadline_selesai','value'=>date('d-m-y',time($row_order['tgl_selesai']))))?></td>
											</tr>
										</table>
									</fieldset>
								</td>
				...
				<div id='gridwrapper' class='gridwrapper'>
					<table style='float:left;border-collapse: collapse; border: 1px solid silver; width: 100%;' border='1'>
						...
						<?php $i=0; ?>
						<?php foreach($query_detail->result_array() as $row) { ?>
							<?php
								$row_produk_kategori 	= $this->db->query('select * from sim_produk_kategori where no_id = '.$row['no_kategori'])->row_array(); 
								$row_produk_jenis 		= $this->db->query('select * from sim_produk_jenis where no_id = '.$row['no_jenis'])->row_array();
								$row_produk 					= $this->db->query('select * from sim_produk where no_id = '.$row['no_produk'])->row_array();
							?>
							<tr>
								<td class='table-row-center'><?php echo label_for(++$i) ?></td>
								<td class='table-row-left'><?php echo label_for($row_produk_kategori['kategori']) ?></td>
								<td class='table-row-left'><?php echo label_for($row_produk_jenis['jenis']) ?></td>
								<td class='table-row-left'><?php echo label_for($row_produk['nama']) ?></td>
								<td class='table-row-right'><?php echo label_for($row['ukuran']) ?></td>
								<td class='table-row-right'><?php echo label_for(number_format($row['harga'],2,'.',',')) ?></td>
								<td class='table-row-right'><?php echo label_for($row['jumlah']) ?></td>
								<td class='table-row-right'><?php echo label_for($row['diskon']) ?></td>
								<td class='table-row-right'><?php echo label_for(number_format($row['total'],2,'.',',')) ?></td>
								<td class='table-row-center'><?php echo button_tag(array('value'=>'hapus','onclick'=>"remove_me(event,$i)")) ?></td>
							</tr>
						<?php } ?>
					</table>
					...
					<?php $i=0 ?>
					<?php foreach($query_detail->result_array() as $row) { ?>
						<?php $row_produk = $this->db->query('select * from sim_produk where no_id = '.$row['no_produk'])->row_array() ?>
						<?php $i++ ?>
						update_bucket("<?php echo $i ?>",<?php echo $row['no_kategori'] ?>,<?php echo $row['no_jenis'] ?>,<?php echo $row['no_produk'] ?>,
							"<?php echo $row['ukuran'] ?>","<?php echo $row['harga'] ?>",<?php echo $row['jumlah'] ?>,
							<?php echo $row['diskon'] ?>,"<?php echo $row['total'] ?>");
								...
								<table style='float:right;'>
									<tr>
										<td>
											...
											'value'=>'Update','onclick'=>'button_simpan_transaksi_onClick(event)'))?>
										</td>
										<td>
								...
								<fieldset>
									...
									<table style='float:left;'>
										...
										<tr>
											...
											<td><?php echo label_for(':')?></td>
											<td colspan='2'><?php echo textbox_tag(array('id'=>'jumlah_uang','name'=>'jumlah_uang','style'=>'text-align:right;width:180px',
											'value'=>$row_order['jumlah_uangmuka'],
											'onkeyup'=>'jumlah_uang_onKeyUp(event)'))?></td>
										</tr>
										<tr>
								...
								<fieldset>
									<table style='float:left;'>
										<tr>
											...
											<td><?php echo label_for(':')?></td>
											<td><?php echo textbox_tag(array('id'=>'total','name'=>'total','style'=>'text-align:right;width:180px',
											'readonly'=>'true','value'=>number_format($row_order['total'],2,'.',',')))?></td>
										</tr>
										<tr>
											<td><?php echo label_for('Biaya Tambahan')?></td>
											<td><?php echo label_for(':')?></td>
											<td><?php echo textbox_tag(array('id'=>'biaya_tambahan','name'=>'biaya_tambahan','style'=>'text-align:right;width:180px',
											'onkeyup'=>'biaya_tambahan_onKeyUp(event)','value'=>$row_order['biaya_tambahan']))?></td>
										</tr>
										<tr>
											<td><?php echo label_for('Jumlah Tagihan')?></td>
											<td><?php echo label_for(':')?></td>
											<td><?php echo textbox_tag(array('id'=>'jumlah_tagihan','name'=>'jumlah_tagihan','style'=>'text-align:right;width:180px',
											'readonly'=>'true','value'=>number_format($row_order['jumlah_tagihan'],2,'.',',')))?></td>
										</tr>
										<tr>
											<td><?php echo label_for('Sisa Tagihan')?></td>
											<td><?php echo label_for(':')?></td>
											<td><?php echo textbox_tag(array('id'=>'sisa_tagihan','name'=>'sisa_tagihan','style'=>'text-align:right;width:180px',
											'readonly'=>'true','value'=>number_format($row_order['sisa'],2,'.',',')))?></td>
										</tr>
									</table>
								</fieldset>
							</td>
