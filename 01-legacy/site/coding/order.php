<?php
	...
	switch (mysqli_real_escape_string($konek,$_POST['submit'])){
		case "ORDER NOW":
			$id=amankan($_GET['id']);
			...
			if(!isset($id) && !isset($slug)){
				...
				if(isset($error)){
					...
					$q=mysqli_query($konek,"SELECT * FROM produk WHERE id='$id' AND slug='$slug' LIMIT 1")or die(mysqli_error());
		...
		case "finish":
					...
					if(isset($error)){
						...
						$quo	= mysqli_query($qx, $konek)or die(mysqli_error());
						
						if($quo){
							$li		= mysqli_insert_id($konek);
							...
							$qin 	= mysqli_query($konek,$inso)or die(mysqli_error());
